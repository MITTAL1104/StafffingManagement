...
const UpdateEmployee = () => {
  ...
  const handleDropdownSelect = (selectedName) => {
    if (selectedName === "No matching employees found" || selectedName === "Error fetching employees") {
      return;
    }
  ...
  const fetchEmployeeData = async () => {
    if (!inputValue.trim()) {
      showMessage("Please enter Employee Name!", true);
      return;
    }

    if(!selectedName){
        showMessage("Please select an employee from dropdown!",true);
        return;
    }

    setIsFetching(true);
    try {
      const response = await fetchByNameForUpdate("employee", selectedName);
      if (response) {
        setFormData(response);
        showMessage("Employee data loaded successfully");
        setDropdownVisible(false);
        setInputValue(response.name); // Only show name after loading
      } else {
            ...
            <div className="dropdown">
              {allEmployees.map((emp, index) => (
                <div
                  ...
                  onClick={() => handleDropdownSelect(emp.name)}
                >
